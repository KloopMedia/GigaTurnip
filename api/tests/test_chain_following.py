...
from api.models import CustomUser, TaskStage, Campaign, Chain, ConditionalStage, Stage, Rank, RankRecord, RankLimit, \
    Task, CopyField, Integration, ResponseFlattener
...
class GigaTurnip(APITestCase):
    ...
    def get_csv(self, response_flattener_id, stage_id=None, client=None):
        s_id = stage_id
        if not s_id:
            s_id = self.initial_stage.id
        c = client
        if not c:
            c = self.client
        url = reverse('task-csv')
        response = c.get(url + f"?stage={s_id}&response_flattener={response_flattener_id}")
        return response
