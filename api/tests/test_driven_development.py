...
class NotificationTest(APITestCase):
    def setUp(self):
        ...
        self.another_task_stage = TaskStage.objects.create(name="Task stage", x_pos=1, y_pos=1,
                                                   chain=self.another_chain)
        self.another_rank = Rank.objects.create(name="new rank for new user")
        self.rank_limit = RankLimit.objects.create(rank=self.another_rank, stage=self.another_task_stage,
                                                   open_limit=3, total_limit=4)
        self.another_notification = [Notification.objects.create(title=f"HI!{i}", text=f'Hello world!#{i}', rank=self.another_rank,
    ...
    def test_retrieve_manager_success(self):
        ...
        self.assertEqual(NotificationStatuses.objects.filter(user=self.user).filter(viewed=True).count(), 0)

    # user with the same rank retrieve his notification
    def test_retrieve_user_with_same_rank_success(self):
        self.new_user.managed_campaigns.add(self.campaign)
        RankRecord.objects.create(user=self.user, rank=self.rank)
        self.assertEqual(NotificationStatuses.objects.count(), 0)
        ...
        self.assertEqual(NotificationStatuses.objects.filter(user=self.user).filter(viewed=True).count(), 5)
