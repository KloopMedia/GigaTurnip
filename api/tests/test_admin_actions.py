...
class CustomUserAdminTest(TestCase):
    ...
    def test_set_rank(self):
        ...
        self.assertTrue(bool(RankRecord.objects.filter(user=self.employee.id, rank=self.rank)))
...
class TaskAdminTest(TestCase):
    ...
    def test_mark_complete_fail(self):
        ...
        self.client.login(username=self.user.username, password=self.u_password)
        response = self.client.post(self.url, data, follow=True)
        self.assertFalse(Task.objects.get(id=task.id).complete)
    ...
    def test_mark_complete_success(self):
        ...
        data = {'action': 'make_completed', '_selected_action': [task.id]}

        self.client.login(username=self.user.username, password=self.u_password)
        response = self.client.post(self.url, data, follow=True)
        self.assertTrue(Task.objects.get(id=task.id).complete)

    def test_mark_complete_force_fail(self):
        case = Case.objects.create()
    ...
    def test_mark_complete_force_success(self):
        case = Case.objects.create()
        tasks = [Task.objects.create(assignee=self.new_user, stage=self.task_stage, case=case,
                                     complete=False) for x in range(5)]
        tasks_complete = tasks[:3]
