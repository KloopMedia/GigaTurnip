...
class DanceVocabreTest(GigaTurnipTestHelper):
    def test_task_own_schema(self):
        ...
        self.initial_stage.json_schema = stage_schema
        self.initial_stage.ui_schema = stage_ui_schema
    ...
    def test_task_stage_schema(self):
        ...
        response = self.get_objects("task-detail", pk=task.pk)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["stage"]["json_schema"], stage_schema)
        self.assertEqual(response.data["stage"]["ui_schema"], stage_ui_schema)
