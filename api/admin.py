...
from .models import Campaign, Chain, \
    ...
    ResponseFlattener, TaskAward, DynamicJson, PreviousManual, AutoNotification, ConditionalLimit, DatetimeSort, Error
...
class ErrorAdmin(admin.ModelAdmin):
    model = Error
    list_display = ('campaign', 'title', 'details', )
    list_filter = ('campaign', )
    ...
    def get_queryset(self, request):
        queryset = super(ErrorAdmin, self).get_queryset(request).select_related('campaign')
